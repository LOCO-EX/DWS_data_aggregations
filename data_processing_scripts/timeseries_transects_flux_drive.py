...
START_DATE = 0
# Number of years to process starting from the start date
YEARS = 0
...
def create_volume_flux_das(transect: str, file: Path):
    ...
    # log
    print(f"Transect name: {transect}")
...
def create_merged_das(transects: list[str], path_files: str | Path):
    ...
    for j in range(0, YEARS):
        ...
        for i in range(0, 12):  # for processing the whole year
            ...
            # log
            print(f"Processing date: {date_full}")
...
def timeseries_transects_flux(
    ...
    if save:
        ds.to_netcdf(
            Path(PATH_ROOT / path_output / f"TR.volume_salt_flux.nc").resolve(),
            format="NETCDF4",
            encoding=encoding,
        )
