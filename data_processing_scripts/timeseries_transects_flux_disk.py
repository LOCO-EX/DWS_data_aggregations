...
def create_volume_flux_das(transect: str, file: Path):
    ...
    if "uu" in vars and "vv" in vars:
        if transect == "Eierlandsgat":
            ...
            if xc.shape[0] != 1 and yc.shape[0] != 1:
                print(
                ...
                print(f"Spacing mean: {np.mean(np.sqrt((yc[1:]-yc[:-1])**2))}")
                print(
                    f"Spacing std (should be around zero): {np.std(np.sqrt((yc[1:]-yc[:-1])**2))}"
                )
            else:
                print(f"Spacing mean: {np.mean(np.sqrt((xc[1:]-xc[:-1])**2))}")
                print(
                    f"Spacing std (should be around zero): {np.std(np.sqrt((xc[1:]-xc[:-1])**2))}"
                )
            quit()

        # # TEST to check if the spacing is homogeneous
        # if xc.shape[0] != 1 and yc.shape[0] != 1:
        #     print(
        #         f"Spacing mean: {np.mean(np.sqrt((xc[1:]-xc[:-1])**2+(yc[1:]-yc[:-1])**2))}"
        #     )
        ...
        # point_width = 200  # approximated value
...
def create_merged_das(transects: list[str], path_files: str | Path):
    ...
    for j in range(0, YEARS):
        ...
        for i in range(0, 12):  # for processing the whole year
            date_full = date + i * 100
            for transect in transects:
                file = (
                    "TR." + transect + ".3d." + str(date_full) + ".nc"
                )  # All the different time
                da_vol_flux, da_salt_flux, da_n, da_coords_1, da_coords_2 = (
                    create_volume_flux_das(transect, path_files / str(date_full) / file)
                )
...
def timeseries_transects_flux(
    ...
    path_files = PATH_ROOT / path_files
    ...
    if save:
        ds.to_netcdf(
            Path(
                PATH_ROOT
                / path_output
                / f"TR.volume_salt_flux.{START_DATE}-{END_DATE}.nc"
            ).resolve(),
            format="NETCDF4",
            encoding=encoding,
        )
